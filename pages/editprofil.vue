<template>
  <div class="xl:mx-64 px-4">
    <div class="text-center font-sans mx-auto mt-10 mb-16 lg:mb-0">
      <h1 class="text-teal-700">
        <span class="font-extrabold text-5xl xs:text-4xl sm:text-5xl md:text-5xl lg:text-5xl xl:text-5xl 2xl:text-5xl">Update</span>
        <span class="font-semibold text-4xl xs:text-3xl sm:text-4xl md:text-4xl lg:text-4xl xl:text-4xl 2xl:text-4xl">Profil</span>
      </h1>
    </div>
    <div class="max-w-3xl flex items-center h-full lg:h-full flex-wrap mx-auto lg:my-0">
      <!--Main Col-->
      <div id="profile" class="w-full lg:w-3/5 rounded-lg lg:rounded-l-lg lg:rounded-r-none shadow-2xl bg-white lg:mx-0">
        <div class="p-4 md:p-12 text-center lg:text-left">
          <!-- Image for mobile view-->
          <div v-if="$auth.user.foto !== null" class="block lg:hidden rounded-full shadow-xl mx-auto -mt-16 h-32 w-32 bg-cover bg-center border-b-8 border-t-4 border-teal-700" :lazy-background="$auth.user.foto"></div>
          <div v-else class="block lg:hidden rounded-full shadow-xl mx-auto -mt-16 h-32 w-32 bg-cover bg-center">
            <svg class="text-teal-700" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
              <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-6-3a2 2 0 11-4 0 2 2 0 014 0zm-2 4a5 5 0 00-4.546 2.916A5.986 5.986 0 0010 16a5.986 5.986 0 004.546-2.084A5 5 0 0010 11z" clip-rule="evenodd" />
            </svg>
          </div>
          <ValidationObserver ref="observer" v-slot="{ invalid }">
          <form @submit.prevent="submit">
            <ValidationProvider mode="aggressive" rules="required|min:4" v-slot="{ errors }">
              <input
                id="nama"
                type="text"
                placeholder="Nama Anda"
                v-model="update.nama"
                class="xs:w-full sm:max-w-xs md:max-w-sm rounded-lg border-2 border-gray-300 px-2 text-4xl xs:text-2xl sm:text-4xl md:text-4xl lg:text-4xl xl:text-4xl 2xl:text-4xl font-bold mt-8 lg:pt-0 text-teal-700 text-center lg:text-left focus:border-current"
                :class="{ 'border-red-600' : errors[0] }"
              />
              <span class="text-red-600 text-sm">{{ errors[0] }}</span>
            </ValidationProvider>

          <div class="mx-auto lg:mx-0 w-5/6 pt-3 border-b-2 border-teal-700"></div>
          <p class="pt-4 text-sm font-bold flex items-center justify-center lg:justify-start">
            <font-awesome-icon class="text-teal-700 focus:text-teal-800 hover:text-teal-400" :icon="['fas', 'envelope']"/>
            &nbsp;&nbsp;&nbsp;
            {{ update.email }}
          </p>
          <p class="pt-4 text-sm flex items-center justify-center lg:justify-start">
            <font-awesome-icon class="text-teal-700 focus:text-teal-800 hover:text-teal-400" :icon="['fas', 'map-marker-alt']"/>&nbsp;&nbsp;&nbsp;
            <ValidationProvider mode="aggressive" rules="required" v-slot="{ errors }" class="w-1/2 mr-1">
            <v-select tag="input" class="w-full" :options="kota" v-model="update.kota" placeholder="Kota"></v-select>
            <span class="text-red-600 text-sm">{{ errors[0] }}</span>
            </ValidationProvider>
            <ValidationProvider mode="aggressive" rules="required" v-slot="{ errors }" class="w-1/2 ml-1">
            <v-select tag="input" class="w-full" :options="provinsi" v-model="update.provinsi" placeholder="Provinsi"></v-select>
            <span class="text-red-600 text-sm">{{ errors[0] }}</span>
            </ValidationProvider>
          </p>
          <ValidationProvider mode="aggressive" rules="required|minmax:4,100" v-slot="{ errors }">
          <p class="pt-2 text-sm font-bold flex items-center justify-center lg:justify-start">
            <font-awesome-icon class="text-teal-700 focus:text-teal-800 hover:text-teal-400" :icon="['fas', 'map-marker-alt']"/>&nbsp;&nbsp;&nbsp;
            <textarea placeholder="Alamat" class="w-full border-2 border-gray-300 rounded-lg placeholder-black p-2" v-model="update.alamat"></textarea>
          </p>
          <span class="text-red-600 text-sm w-full text-center">{{ errors[0] }}</span>
          </ValidationProvider>
          <ValidationProvider mode="aggressive" rules="required|numeric|minmax:3,13" v-slot="{ errors }">
          <p class="pt-2 text-sm font-bold flex items-center justify-center lg:justify-start">
            <font-awesome-icon class="text-teal-700 focus:text-teal-800 hover:text-teal-400" :icon="['fas', 'phone']"/>&nbsp;&nbsp;&nbsp;
            <input
                id="no_telepon"
                type="text"
                placeholder="No Telepon"
                v-model="update.no_telepon"
                class="w-full border-2 border-gray-300 rounded-lg placeholder-black p-2 text-left"
                :class="{ 'border-red-600' : errors[0] }"
                />
          </p>
          <span class="text-red-600 text-sm">{{ errors[0] }}</span>
          </ValidationProvider>
          <ValidationProvider mode="aggressive" rules="required" v-slot="{ errors }">
          <p class="pt-2 text-sm font-bold flex items-center justify-center lg:justify-start">
            <font-awesome-icon class="text-teal-700 focus:text-teal-800 hover:text-teal-400" :icon="['fas', 'user-graduate']"/>&nbsp;&nbsp;&nbsp;
            <v-select tag="input" class="w-full" :options="pendidikan" v-model="update.pendidikan" placeholder="Pendidikan"></v-select>
          </p>
          <span class="text-red-600 text-sm w-full text-center">{{ errors[0] }}</span>
          </ValidationProvider>
          <textarea class="border-2 w-full mt-4 p-2 text-sm" v-model="update.bio" placeholder="Tulis beberapa kata tentangmu...(Opsional)"></textarea>
          <div class="pt-12 pb-8">
            <button
              :disabled="invalid"
              class="bg-teal-700 hover:bg-teal-900 text-white font-bold py-2 px-3 rounded-r-none rounded-full disabled:opacity-50">
              <font-awesome-icon class="text-white" :icon="['fas', 'save']"/> Simpan
            </button>
            <a @click="upload"
              class="bg-teal-700 border-l-2 border-white hover:bg-teal-900 text-white font-bold px-3 -ml-1 rounded-full rounded-l-none disabled:opacity-50 cursor-pointer" style="padding-top:9px;padding-bottom:9px;">
              <font-awesome-icon class="text-white" :icon="['fas', 'camera']"/>
            </a>
          </div>
          </form>
          </ValidationObserver>
          <div class="mt-6 pb-16 lg:pb-0 w-4/5 lg:w-full mx-auto flex flex-wrap items-center justify-between">
            <a
              target="_blank"
              class="link"
              href="#"
              data-tippy-content="@facebook_handle">
              <svg
                class="h-6 fill-current text-gray-600 hover:text-teal-700"
                role="img"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg">
                <title>Facebook</title>
                <path
                d="M22.676 0H1.324C.593 0 0 .593 0 1.324v21.352C0 23.408.593 24 1.324 24h11.494v-9.294H9.689v-3.621h3.129V8.41c0-3.099 1.894-4.785 4.659-4.785 1.325 0 2.464.097 2.796.141v3.24h-1.921c-1.5 0-1.792.721-1.792 1.771v2.311h3.584l-.465 3.63H16.56V24h6.115c.733 0 1.325-.592 1.325-1.324V1.324C24 .593 23.408 0 22.676 0"/>
                </svg>
            </a>
            <a
              target="_blank"
              class="link"
              href="#"
              data-tippy-content="@twitter_handle">
              <svg
                class="h-6 fill-current text-gray-600 hover:text-teal-700"
                role="img"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg">
                <title>Twitter</title>
                <path
                d="M23.954 4.569c-.885.389-1.83.654-2.825.775 1.014-.611 1.794-1.574 2.163-2.723-.951.555-2.005.959-3.127 1.184-.896-.959-2.173-1.559-3.591-1.559-2.717 0-4.92 2.203-4.92 4.917 0 .39.045.765.127 1.124C7.691 8.094 4.066 6.13 1.64 3.161c-.427.722-.666 1.561-.666 2.475 0 1.71.87 3.213 2.188 4.096-.807-.026-1.566-.248-2.228-.616v.061c0 2.385 1.693 4.374 3.946 4.827-.413.111-.849.171-1.296.171-.314 0-.615-.03-.916-.086.631 1.953 2.445 3.377 4.604 3.417-1.68 1.319-3.809 2.105-6.102 2.105-.39 0-.779-.023-1.17-.067 2.189 1.394 4.768 2.209 7.557 2.209 9.054 0 13.999-7.496 13.999-13.986 0-.209 0-.42-.015-.63.961-.689 1.8-1.56 2.46-2.548l-.047-.02z"/>
                </svg>
            </a>
            <a
              target="_blank"
              class="link"
              href="#"
              data-tippy-content="@instagram_handle">
              <svg
                class="h-6 fill-current text-gray-600 hover:text-teal-700"
                role="img"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg">
                <title>Instagram</title>
                <path
                  d="M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913.306.788.717 1.459 1.384 2.126.667.666 1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558.788-.306 1.459-.718 2.126-1.384.666-.667 1.079-1.335 1.384-2.126.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913-.306-.789-.718-1.459-1.384-2.126C21.319 1.347 20.651.935 19.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0zm0 2.16c3.203 0 3.585.016 4.85.071 1.17.055 1.805.249 2.227.415.562.217.96.477 1.382.896.419.42.679.819.896 1.381.164.422.36 1.057.413 2.227.057 1.266.07 1.646.07 4.85s-.015 3.585-.074 4.85c-.061 1.17-.256 1.805-.421 2.227-.224.562-.479.96-.899 1.382-.419.419-.824.679-1.38.896-.42.164-1.065.36-2.235.413-1.274.057-1.649.07-4.859.07-3.211 0-3.586-.015-4.859-.074-1.171-.061-1.816-.256-2.236-.421-.569-.224-.96-.479-1.379-.899-.421-.419-.69-.824-.9-1.38-.165-.42-.359-1.065-.42-2.235-.045-1.26-.061-1.649-.061-4.844 0-3.196.016-3.586.061-4.861.061-1.17.255-1.814.42-2.234.21-.57.479-.96.9-1.381.419-.419.81-.689 1.379-.898.42-.166 1.051-.361 2.221-.421 1.275-.045 1.65-.06 4.859-.06l.045.03zm0 3.678c-3.405 0-6.162 2.76-6.162 6.162 0 3.405 2.76 6.162 6.162 6.162 3.405 0 6.162-2.76 6.162-6.162 0-3.405-2.76-6.162-6.162-6.162zM12 16c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4zm7.846-10.405c0 .795-.646 1.44-1.44 1.44-.795 0-1.44-.646-1.44-1.44 0-.794.646-1.439 1.44-1.439.793-.001 1.44.645 1.44 1.439z"/>
              </svg>
            </a>
            <a
              target="_blank"
              class="link"
              href="#"
              data-tippy-content="@youtube_handle">
              <svg
                class="h-6 fill-current text-gray-600 hover:text-teal-700"
                role="img"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24">
                <title>YouTube</title>
                <path
                  d="M23.495 6.205a3.007 3.007 0 0 0-2.088-2.088c-1.87-.501-9.396-.501-9.396-.501s-7.507-.01-9.396.501A3.007 3.007 0 0 0 .527 6.205a31.247 31.247 0 0 0-.522 5.805 31.247 31.247 0 0 0 .522 5.783 3.007 3.007 0 0 0 2.088 2.088c1.868.502 9.396.502 9.396.502s7.506 0 9.396-.502a3.007 3.007 0 0 0 2.088-2.088 31.247 31.247 0 0 0 .5-5.783 31.247 31.247 0 0 0-.5-5.805zM9.609 15.601V8.408l6.264 3.602z"/>
              </svg>
            </a>
          </div>
          <!-- Use https://simpleicons.org/ to find the svg for your preferred product -->
        </div>
      </div>
      <!--Img Col-->
      <div class="w-full lg:w-2/5">
        <!-- Big profile image for side bar (desktop) -->
        <img v-if="$auth.user.foto !== null" :data-src="$auth.user.foto" class="rounded-none lg:rounded-xl lg:rounded-l-none shadow-2xl hidden lg:block" alt="profilePicture" title="profilePicture" v-lazy-load/>
        <img v-else data-src="~/assets/samp_pp.png" class="rounded-none lg:rounded-xl lg:rounded-l-none shadow-2xl hidden lg:block" alt="defaultProfilePicture" title="defaultProfilePicture" v-lazy-load/>
        <!-- Image from: http://unsplash.com/photos/MP0IUfwrn0A -->
      </div>
    </div>
  </div>
</template>

<script>
import { ValidationProvider, ValidationObserver } from "vee-validate";

export default {
  auth: true,
  components: {
    ValidationProvider, ValidationObserver
  },
  data() {
    return {
      kota: [
        'Surabaya',
        'Malang',
        'Tulungagung',
        'Trenggalek'
      ],
      provinsi: [
        'DKI Jakarta',
        'Jawa Barat',
        'Jawa Tengah',
        'Jawa Timur',
        'Banten',
      ],
      pendidikan: [
        'SD',
        'SMP',
        'SMA-10',
        'SMA-11',
        'SMA-12',
      ],
      update: {
        email: this.$auth.user.email,
        nama: this.$auth.user.nama,
        kota: this.$auth.user.kota,
        provinsi: this.$auth.user.provinsi,
        alamat: this.$auth.user.alamat,
        pendidikan: this.$auth.user.pendidikan,
        no_telepon: this.$auth.user.no_telepon,
        bio: this.$auth.user.bio,
      }
    }
  },
  head() {
    return {
      title: this.$auth.user.nama+' - Edit Profil',
      meta: [
        { charset: 'utf-8' },
        { name: 'viewport', content: 'width=device-width, initial-scale=1, maximum-scale=1.0, user-scalable=0' },
        { hid: 'description', name: 'description', content: this.$auth.user.bio },
        { name: 'theme-color', content: '#006d71' },
        { name: "msapplication-TileImage", content: "/locig.png" },
        { name: "msapplication-TileColor", content: "#006d71" },
      ],
    }
  },
  methods: {
    upload(){
      alert('Coming Soon')
    },
    async submit() {
      const isValid = await this.$refs.observer.validate();
      if (!isValid) {
        this.$toast.show({
          type:'danger',
          title: 'Form tidak sesuai',
          message: 'Periksa form yang anda isi!',
        })
      }else{
        this.updating()
      }
    },
    updating() {
      this.$axios.get('/bimapi/sanctum/csrf-cookie').then(response => {
        this.$axios.post('/bimapi/api/user/updateProfil', this.update)
        .then(response => { this.success() })
        .catch(error => console.log(error))
      })
      .catch(error => console.log(error))
    },
    success() {
      this.$toast.show({
        type:'success',
        title: 'Update Berhasil',
        message: 'Hai, '+this.$auth.user.nama+' data anda berhasil di Update!',
      })
      .then(respone => {this.$routes.push('/profil')})
    }
  },
}
</script>
